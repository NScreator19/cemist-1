...
# Tabel Sebelum
st.subheader("📋 Kondisi Sebelum (Tipe Semen yang Dipilih)")
df_sebelum = df[df["Cement type"].isin(semen_dipilih)][["Cement type", "Cement Production (ton)", "Clinker Consumption (ton)", "Clinker Factor (%)"]]
st.dataframe(df_sebelum.style.format({"Clinker Factor (%)": "{:.2f}", "Clinker Consumption (ton)": "{:,.0f}"}))

# Tabel Sesudah
st.subheader("📊 Kondisi Setelah Penyesuaian (Tipe Semen yang Dipilih)")
df_sesudah = df_result[df_result["Cement type"].isin(semen_dipilih)][["Cement type", "Cement Production (ton)", "Clinker Consumption (ton)", "Clinker Factor (%)"]]
st.dataframe(df_sesudah.style.format({"Clinker Factor (%)": "{:.2f}", "Clinker Consumption (ton)": "{:,.0f}"}))
